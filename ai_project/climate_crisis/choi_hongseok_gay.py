...
story = [
    ...
    ("2. 연구기지 도착", "남쪽으로 간 태민은 주변에서 연구기지를 발견합니다. 하지만 연구기지 또한 온실가스의 영향으로 연구기지 쪽 얼음이 녹고 있었습니다. 연구기지에서 생존한 최홍석이 도움을 요청합니다. 최홍석은 6수를 한 파렴치한 부모님 등골브레이커지만, 끝내 배재대학교에 입학한 개상노무새끼입니다. 어떤 도움을 주시겠습니까?", 
    ...
    ("5. 자원 부족과 갈등", "명석의 노예가 된 태민은 연구기지에 있는 자원을 닥치는 대로 쓰고 있습니다. 그러한 태민의 태도에 분노한 명석과 태민 간에 자원 부족으로 인한 갈등이 시작됩니다. 태민은 이를 조절하고자 하지만, 상황은 점점 악화되고 있습니다. 어떻게 해결하시겠습니까?", 
        ["1: 나가서 자원 구해오기", "2: 자원을 챙겨서 이동하기."]),
    ("6. 구조를 위한 노력", "연구기지를 떠난 태민과 명석은 탐사선을 찾았습니다. 비록 얼음이 녹아내려 바다에 떠있어서 작동은 하지 않겠지만 구조요청은 보낼 수 있을 것입니다. 태민은 명석과 함께 구조를 위한 신호를 보내려고 합니다. 바다에 떠있는 탐사선까지 어떻게 가실 것입니까?", 
        ["1: 태민이 수영해서 구조신호를 보낸다.", "2: 명석이 수영해서 구조신호를 보낸다."]),
    ("7. 결말", "다행히 수영을 할 줄 알던 태민이 탐사선까지 도착해서 구조신호를 보냈습니다. 곧이어 구조선이 와서 태민은 어려운 여정 끝에 구조되어 생존을 이끌어냈습니다. 그는 자신의 경험을 통해 온실 가스 배출과 환경 파괴로 인한 위험성을 알리고자 합니다. 이를 통해 태민은 지구 환경 보호의 중요성을 널리 알립니다.")
]
...
def main():
    ...
    while running:
        ...
        # 이야기 텍스트를 30글자 단위로 줄 바꿈하여 표시
        text = story[current_stage][1]
        x, y = 20, 70
        while text:
            display_text(text[:30], x, y)
            text = text[30:]
            y += 30

        # 선택지 표시
        if current_stage < len(story) - 1 and current_stage % 2 == 0:  
            display_choices(story[current_stage + 1][1], y + 50)
