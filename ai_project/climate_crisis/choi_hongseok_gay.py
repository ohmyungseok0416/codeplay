...
def display_choices(choice1, choice2):
    text_surface1, text_rect1 = display_text(choice1, WIDTH // 4, HEIGHT - 100)
    text_surface2, text_rect2 = display_text(choice2, WIDTH * 3 // 4, HEIGHT - 100)
    return (text_surface1, text_rect1), (text_surface2, text_rect2)

# 게임 루프 안에서 선택지 표시 및 이벤트 처리 부분 수정
running = True  # running 변수를 루프 밖에서 정의하고 초기값을 True로 설정

while running:
    win.fill(WHITE)

    # 이전 코드와 마찬가지로 스토리와 제목 표시 부분을 유지합니다.

    # 선택지 표시
    if current_stage < len(choices):
        (choice1_surface, choice1_rect), (choice2_surface, choice2_rect) = display_choices(choices[current_stage][0], choices[current_stage][1])
        win.blit(choice1_surface, choice1_rect)
        win.blit(choice2_surface, choice2_rect)
    ...
    if selected_choice is not None:
        ...
        if current_stage < len(choices):
            ...
            # current_stage가 스토리 길이를 초과하지 않도록 보정
            current_stage = min(current_stage, len(story) - 1)

        # 선택 처리가 끝나면 다시 None으로 초기화
